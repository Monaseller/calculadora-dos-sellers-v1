/**
 * `consultar_vendas` — CONTRATO PURO, sem executor.
 *
 * ── Por que este arquivo existe ─────────────────────────────────────
 *
 * O caminho automatico e a retomada pos-aprovacao precisam preparar a
 * MESMA entrada e interpretar o MESMO resultado. Se cada um tivesse a
 * sua copia, as duas teriam de concordar para sempre — e um bugfix
 * futuro corrigiria uma e esqueceria a outra, exatamente o defeito que
 * a primitiva compartilhada do D5 evitou do lado do banco.
 *
 * Entao a implementacao mudou de LUGAR, nao de CONTEUDO: tudo abaixo
 * saiu de `consultar-vendas.ts` por extracao literal. O handler
 * continua sendo o executor automatico e reexporta estas MESMAS
 * referencias; `resume-contratos.ts` aponta para elas tambem.
 *
 * ── A barreira que faz o grafo ser aciclico ─────────────────────────
 *
 * Este modulo NAO importa `executarFuncao`, nem o registry, nem worker,
 * nem cliente de banco, nem env, nem rede. `ResultadoExecucaoFuncao`
 * entra por `import type`, que some na compilacao e por isso nao cria
 * aresta de runtime. E o que permite `resume-contratos` alcancar a
 * preparacao e a continuacao sem passar pelo handler que executa.
 */
