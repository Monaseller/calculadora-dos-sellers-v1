...
function getRange(periodo: Periodo): { from: string; to: string } {
  const hoje = hojeISO();
  if (periodo === "hoje")  return { from: hoje, to: hoje };
  if (periodo === "7dias") return { from: addDays(hoje, -6), to: hoje };
  // mes atual
  const [y, m] = hoje.split("-");
  return { from: `${y}-${m}-01`, to: hoje };
}
...
function BarChart({ dias }: { dias: DiaData[] }) {
  ...
  return (
    <div style={{ width: "100%", overflowX: "auto" }}>
      <div style={{ minWidth: dias.length > 10 ? `${dias.length * 48}px` : "100%", height: "180px", display: "flex", alignItems: "flex-end", gap: "4px", padding: "0 4px" }}>
        {dias.map((d, i) => {
...
// ── Card KPI ──────────────────────────────────────────────────────────────
function KpiCard({ label, valor, sub, cor = "#fff" }: { label: string; valor: string; sub?: string; cor?: string }) {
  return (
    <div style={{
...
export default function DashboardPage() {
  ...
  const carregar = useCallback(async (p: Periodo) => {
    ...
    try {
      ...
      const pedidosSet = new Set(rows.map(r => r.orderId));
      ...
      const diasArr: DiaData[] = Array.from(mapaData.entries())
      ...
      // ── Top anúncios ──────────────────────────────────────────────────
      const mapaAnuncios = new Map<string, TopAnuncio>();
      for (const r of rows) {
        const key = r.mlItemId || r.anuncio;
        const ex  = mapaAnuncios.get(key);
        if (ex) {
          ex.faturamento += r.faturamento;
          ex.lucro       += r.margemContrib;
          ex.qtd         += r.qtd;
        } else {
          mapaAnuncios.set(key, {
            mlItemId: r.mlItemId,
            nome:     r.anuncio,
            sku:      r.sku,
            faturamento: r.faturamento,
            lucro:    r.margemContrib,
            qtd:      r.qtd,
            margem:   0,
          });
        }
      ...
      const topsArr = Array.from(mapaAnuncios.values())
  ...
  const labelPeriodo = periodo === "hoje" ? "hoje" : periodo === "7dias" ? "nos últimos 7 dias" : "neste mês";
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "28px", flexWrap: "wrap", gap: "12px" }}>
        <div>
          ...
          <p style={{ fontSize: "13px", color: "#9099aa", marginTop: "4px" }}>
            Visão geral das suas vendas {labelPeriodo}
          </p>
        </div>

        {/* Seletor de período */}
        <div style={{ display: "flex", background: "rgba(255,255,255,0.05)", borderRadius: "10px", padding: "4px", gap: "2px" }}>
          {([["hoje", "Hoje"], ["7dias", "7 dias"], ["mes", "Mês"]] as [Periodo, string][]).map(([val, label]) => (
            <button
              key={val}
              onClick={() => setPeriodo(val)}
              style={{
                padding: "7px 16px", borderRadius: "7px", border: "none",
                background: periodo === val ? "rgba(255,182,0,0.18)" : "transparent",
                color: periodo === val ? "#FFB600" : "#9099aa",
      ...
      {!loading && !erro && kpis && (
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: "14px", marginBottom: "24px" }}>
            <KpiCard
              label="Faturamento"
              valor={fmtBRL(kpis.faturamento)}
              sub={`${kpis.unidades} unid.`}
              cor="#FFB600"
            />
            <KpiCard
              label="Pedidos"
            ...
            <KpiCard
              label="Ticket Médio"
              valor={fmtBRL(kpis.ticket)}
              sub="por pedido"
            />
            <KpiCard
              label="Lucro Líquido"
              valor={fmtBRL(kpis.lucro)}
              cor={kpis.lucro >= 0 ? "#00D97E" : "#ff6b6b"}
              sub="após custos"
            />
            <KpiCard
              label="Margem"
            ...
            <div style={{
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "2px" }}>
                {tops.map((a, i) => (
                  ...
                  >
                    {/* Rank */}
                    <span style={{ fontSize: "13px", fontWeight: 800, color: i < 3 ? "#FFB600" : "#9099aa" }}>
                      {i + 1}
                    </span>
                    ...
                    <div style={{ minWidth: 0 }}>
                      ...
                      {a.sku && (
                        <div style={{ fontSize: "11px", color: "#9099aa", marginTop: "2px" }}>SKU: {a.sku}</div>
                      )}
                    </div>
                    ...
                    {/* Lucro */}
                    <div style={{ textAlign: "right" }}>
                      <div style={{ fontSize: "13px", fontWeight: 700, color: a.lucro >= 0 ? "#00D97E" : "#ff6b6b" }}>
                        {fmtBRL(a.lucro)}
                      </div>
                    ...
                    <div style={{ textAlign: "right" }}>
                      <div style={{
                        ...
                        color: a.margem >= 20 ? "#00D97E" : a.margem >= 10 ? "#FFB600" : "#ff6b6b",
                      }}>
                        {a.margem.toFixed(1)}%
                      </div>
