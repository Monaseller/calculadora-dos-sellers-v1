-- ────────────────────────────────────────────────────────────────────
-- 20260815_geracao_prompts_imagem_job_origem.sql
--
-- Substitui estudio_anuncios_pipeline_avancar() para dar origem
-- deterministica a geracao_prompts_imagem.
--
...
-- NAO necessariamente o job imediatamente anterior na cadeia.
--
-- Mapa apos esta migration:
--   analise_visual   -> geracao_conteudo        (20260811)
--   geracao_conteudo -> revisao_claude          (20260814)
--   revisao_claude   -> adaptacao_marketplace   (20260814, com precedencia)
--   analise_visual   -> geracao_prompts_imagem  (NOVO)
--
-- POR QUE analise_visual E NAO adaptacao_marketplace. Esta e a primeira
-- transicao em que o job anterior na fila (adaptacao_marketplace) NAO e a
-- origem semantica, e o motivo e de seguranca, nao de estilo:
--   * geracao_prompts_imagem produz instrucoes do que sera DESENHADO.
--     Tudo que pode ser desenhado precisa ser visualmente confirmado.
--   * analise_visual e o unico artefato do Pipeline que classifica cada
--     atributo por `origem` ('produto' / 'embalagem_fisica' /
--     'material_promocional' / 'indeterminado'). E essa classificacao que
--     separa "o produto e verde" de "a arte da embalagem e verde".
--   * geracao_conteudo, revisao_claude e adaptacao_marketplace produzem
--     texto comercial derivado. Usa-los como fonte visual LAVA a
--     classificacao de origem: uma frase de anuncio nao carrega mais a
--     marca de que aquele atributo veio de material promocional. Passar
--     copy de marketplace para um gerador de prompt de imagem aumenta a
--     chance de transformar alegacao textual em elemento visual
--     confirmado — a proibicao central da etapa.
--   * Nada que adaptacao_marketplace produz e desenhavel. Os marketplaces
--     do projeto vem de estudio_anuncios_projetos_marketplace, nao do
--     artefato dela.
-- Consequencia: UMA origem, UM job_origem_id. Nenhuma multi-fonte,
-- nenhuma lista, nenhum ORDER BY criado_em.
--
-- REGRA DE RESOLUCAO (identica as anteriores): exatamente 1 candidato.
--   * 1 analise_visual concluida com resultado -> usa essa
--   * >1 -> RAISE ORIGEM_AMBIGUA
--   * 0  -> NULL, e o handler rejeita explicitamente quando o provedor e
--           real (o caminho fake continua funcionando sem regressao)
--
-- NAO GENERALIZA: geracao_prompts_imagem -> geracao_imagem continua sem
-- vinculo, por decisao explicita — sera definido quando geracao_imagem
-- for implementada de verdade.
--
-- Corpo copiado da versao VIGENTE (20260814). Diferenca: o branch entre
-- etapas amplas, marcado abaixo.
-- ────────────────────────────────────────────────────────────────────
