-- ────────────────────────────────────────────────────────────────────
-- 20260816_geracao_imagem_job_origem.sql
--
-- Substitui estudio_anuncios_pipeline_avancar() para dar origem
-- deterministica a geracao_imagem.
--
...
--   geracao_prompts_imagem -> geracao_imagem           (NOVO)
--
-- POR QUE geracao_prompts_imagem. Aqui, diferente da migration anterior,
-- o job imediatamente anterior na fila E a origem semantica: o artefato
-- consumido por geracao_imagem e exatamente o EnvelopeGeracaoPromptsImagem,
-- de onde sai o prompt final, o negative prompt, a finalidade, a ordem, a
-- proporcao e qual imagem e a principal. geracao_imagem NAO decide
-- estrategia visual -- ela executa o contrato ja produzido.
--
-- As fotos originais do projeto sao usadas como REFERENCIA VISUAL pelo
-- provedor de imagem, mas isso nao muda a semantica: referencia nao e
-- artefato de dominio consumido, e `estudio_anuncios_imagens_origem` nem
-- e produzida por um job. job_origem_id continua sendo UM.
--
-- REGRA DE RESOLUCAO (identica as anteriores): exatamente 1 candidato.
--   * 1 geracao_prompts_imagem concluida com resultado -> usa essa
...
--
-- NAO GENERALIZA: nenhuma origem e definida para calculo_score. Sera
-- definida quando aquela etapa for implementada.
--
-- Corpo copiado da versao VIGENTE (20260815). Diferenca: um IF novo no
-- branch intra-etapa, marcado abaixo.
-- ────────────────────────────────────────────────────────────────────
